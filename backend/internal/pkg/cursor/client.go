...
// NewHTTP2Transport returns the default streaming client (direct connection).
func NewHTTP2Transport() *http.Client {
	client, err := StreamingHTTPClient("")
	if err != nil {
		// unreachable today: direct dialing validates nothing up front
